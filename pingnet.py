...
webmode_timeformat = config_section.get(
    "webmode.timeformat", "%H:%M:%S %d/%m/%Y")
...
p_reti = dict([x for x in config_section.items()
              if x[0].startswith('networks.')])
...
try:
    while 1:
        ...
        if show_topography:
            ...
            for scan_res, typ, addr, pc_desc in zip(scan_results, pc_type, addresses, desc_pc):
                if scan_res != "on" and typ == control_type.Always_active:
                    logging.info(
                        ' '.join(("*** WARNING:DEVICE IS OFF *** =>", addr, pc_desc, scan_res)))
            for scan_res, typ, addr, pc_desc in zip(scan_results, pc_type, addresses, desc_pc):
                if (scan_res == "on" and typ == control_type.to_switch_off):
                    logging.info(
                        ' '.join(("--- TURN OFF BEFORE LEAVING --- =>", addr, pc_desc, scan_res)))
        if gethostnames:
        ...
        if webmode:
            ...
            refreshtime = pause if pause > 0 else 1
            webpage = [f'<html><head><meta http-equiv="refresh" content="{refreshtime}"></head><body>', "Last update: {}".format(
                time.strftime(webmode_timeformat)), "<table border=1 width=100%>"]

            for network in networks:
                num_group_pcs = 1
                webstr = "<tr><td>{}{}</td><td>{}</td>".format(
                    ipv4base, network[0], network[2])
                _str = "{}{}:{} =>".format(ipv4base, network[0], network[2])
                webstr2 = ""
                for index in range(network[1]):
                    ...
                    _str += ' .'
                    webstr2 = ('%s<td style="background: %s; color: %s; width: 25px;">%d</td>' %
                               (webstr2, backgroundcolour, colour, num_group_pcs))
                    num_group_pcs += 1
            ...
            if int(config_section.get("webmode.prettify")):
                ...
                result = etree.tostring(
                    document_root, pretty_print=True, method="html").decode()
